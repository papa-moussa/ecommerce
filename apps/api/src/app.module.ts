    ...
    LoggerModule.forRootAsync({
      ...
      useFactory: (config: ConfigService<AppConfig, true>) =>
        buildLoggerOptions({
          ...
          UNSUBSCRIBE_SECRET: config.get('UNSUBSCRIBE_SECRET', { infer: true }),
          SHIPPING_WEBHOOK_SECRET: config.get('SHIPPING_WEBHOOK_SECRET', { infer: true }),
          HEALTH_TOKEN: config.get('HEALTH_TOKEN', { infer: true }),
          APP_ENCRYPTION_KEY: config.get('APP_ENCRYPTION_KEY', { infer: true }),
        }),
    }),
    PrismaModule,
    SearchModule,
